...
=select id,  count(id)  as 'total de pedidos feitos' from pedidos;
...
select clientes.id, telefone, nome, sum(valor) as 'total gasto' from pedidos right join clientes
 on clientes.id = cliente_id group by nome;
...
select * from pedidos;
select avg(pedidos.id) as media from pedidos;
...
select clientes.nome, itens_pedido.quantidade from itens_pedido
inner join pedidos on itens_pedido.pedido_id = pedidos.id
inner join clientes on pedidos.cliente_id = clientes.id
group by clientes.nome, itens_pedido.quantidade;

-- questão 19 - Qual o pedido mais barato, exibir o nome do cliente e o valor do pedido
select clientes.nome, pedidos.id,min(valor) as 'pedido mais barato' from pedidos
inner join clientes on pedidos.cliente_id = clientes.id;

-- questão 20 -  Liste todos os clientes, mesmo que não tenham feito pedidos, com seus respectivos 
-- pedidos (se houver)
select clientes.nome as nome from clientes
